(function($){
    $(document).ready(function(){
        ...
        */
        ...
        $hello.append('<span id="my-name">제 이름은 전형욱입니다.</span>').eq(0);
        ...
        */
        ...
        $(images).each((idx, el) => {
            ...
            $(`<img src="${el}">`).appendTo(bannerItem);
            bannerItem.appendTo('div.banner-container');
        });
        ...
        */
        ...
        $moveBottom.appendTo($('body:last').eq(0));
        $moveBottom.addClass('move');
        $moveBottom.html('아래로 이동했습니다.')
        ...
        */
        ...
        const $listItems1 = $('ul.list-1').children('li').eq(0);
        for (let i = 4; i > 1; i--) {
            $listItems1.after(`<li>리스트 ${i}</li>`);
        }
        ...
        */
        ...
        const $listItems2 = $('ul.list-2').children('li').eq(0);
        for (let i = 1; i <= 4; i++) {
            $listItems2.before(`<li>리스트 ${i}</li>`);
        }
        ...
        */
        ...
        // 이게 맞는 정답의 제안인지는 잘.... 모르겠네요 하하하하하하
        // boolean문을 추가 사용해주는 방향이 더욱 정확한 알고리즘을 갖추게 만들것 같은데 잘 구상이 되질 않습니다.

        let $startWidth = 100;
        let $endWidth = 500;

        const $botton = $('button.btn-1');
        const $box = $('div.box-1');

        $botton.click(() => {  

            if ($startWidth >= $endWidth) { // 클릭시 현재 너비가 더 클때.
                $endWidth = 0;
                $startWidth -= 100;
            }

            if ($startWidth <= $endWidth) { // 클릭시 현재 너비가 더 작을때.
                $endWidth = 500;
                $startWidth += 100;
            }

            $box.css('width', `${$startWidth}+'px'`);
